...
VTText::VTText(float x, float y, float size, const std::string& text, bool alignCenter, D3DCOLOR color) {
	this->m_X = x;
	this->m_Y = y;
	this->m_Height = size;
	this->m_Text = text;
...
void VTText::Render() {
	...
	else this->m_Font->DrawTextA(NULL, this->m_Text.c_str(), -1, &this->m_FontRect, DT_LEFT | DT_NOCLIP, this->m_Color);
}